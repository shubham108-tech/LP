...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    return (
        <div
            className={`fixed inset-y-0 left-0 text-white z-50 flex flex-col h-[100vh] h-[100dvh] w-[270px] max-w-[85vw] md:w-64 bg-[#12072b] border-r border-purple-950/60 shadow-2xl md:shadow-none transition-transform duration-300 ease-in-out md:translate-x-0 ${
                isOpen ? 'translate-x-0' : '-translate-x-full'
            }`}
        >
            <div className="flex items-center justify-between p-5 sm:p-6 flex-shrink-0">
                <div className="text-xl sm:text-2xl font-black bg-gradient-to-r from-fuchsia-400 via-purple-300 to-pink-400 text-transparent bg-clip-text tracking-tight">
                    LibraryPro
                </div>
                <button
                    onClick={onClose}
                    className="md:hidden p-1.5 rounded-lg bg-purple-900/40 text-purple-300 hover:text-white hover:bg-purple-800/60 transition"
            ...
            <div className="px-5 sm:px-6 mb-2 flex-shrink-0">
                <p className="text-[11px] text-purple-300/60 uppercase tracking-widest font-bold">Menu</p>
            </div>

            <nav className="flex-1 min-h-0 space-y-1 px-3 sm:px-4 overflow-y-auto overscroll-contain touch-pan-y custom-sidebar-scroll">
                {links.map((link) => (
                    <Link
                        key={link.path}
                        to={link.path}
                        onClick={onClose}
                        className={`flex items-center px-3.5 py-2.5 sm:py-3 rounded-xl transition-all duration-200 group ${
                            isActive(link.path)
                    ...
                    >
                        <link.icon className={`mr-3 text-xl flex-shrink-0 ${isActive(link.path) ? 'text-white' : 'text-purple-400 group-hover:text-white'}`} />
                        <span className="font-semibold text-sm sm:text-base">{link.label}</span>
                    </Link>
                ))}
            </nav>

            <div className="p-3 sm:p-4 border-t border-purple-900/40 flex-shrink-0 bg-[#12072b]">
                <Link 
                    to={user?.role === 'admin' ? '/admin/profile' : '/teacher/profile'} 
                    className="px-3 mb-2 flex items-center gap-3 hover:bg-purple-900/30 rounded-xl py-2 transition cursor-pointer" 
                    onClick={onClose}
                >
                    {user?.profile_image ? (
                        <img
                            ...
                            alt={user.name}
                            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-fuchsia-500/40 flex-shrink-0"
                        />
                    ) : (
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-purple-900/50 text-fuchsia-300 flex items-center justify-center font-bold text-sm border-2 border-fuchsia-500/40 flex-shrink-0">
                            {user?.name?.charAt(0).toUpperCase()}
                        </div>
                    )}
                    <div className="overflow-hidden min-w-0 flex-1">
                        <p className="text-sm font-semibold text-white truncate">{user?.name}</p>
                        <p className="text-xs text-purple-300/60 capitalize truncate">{user?.role}</p>
                    </div>
                </Link>
                <button
                    onClick={logout}
                    className="w-full flex items-center px-4 py-2 text-pink-400 hover:bg-pink-500/10 hover:text-pink-300 rounded-xl transition-colors"
                >
                    <RiLogoutBoxLine className="mr-3 text-lg flex-shrink-0" />
